...
class OomSserverSideProcess {
    ...
    static function order ( $request, $columns, $isJoin = false )
    {
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            $orderBy = array();
            $dtColumns = OomSserverSideProcess::pluck( $columns, 'dt' );
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    $binding = OomSserverSideProcess::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
    ...
    {
        $bindings = array();
        $db = OomSserverSideProcess::sql_connect( $sql_details );

        // Build the SQL query string from the request
        $limit = OomSserverSideProcess::limit( $request, $columns );
        $order = OomSserverSideProcess::order( $request, $columns, $joinQuery );
        $where = OomSserverSideProcess::filter( $request, $columns, $bindings, $joinQuery );
        ...
        if($joinQuery){
            ...
            $col = OomSserverSideProcess::pluck($columns, 'db', $joinQuery);
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", OomSserverSideProcess::pluck($columns, 'db'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
        ...
        $data = OomSserverSideProcess::sql_exec( $db, $bindings,$query);

        // Data set length after filtering
        $resFilterLength = OomSserverSideProcess::sql_exec( $db,
            "SELECT FOUND_ROWS()"
        );
        ...
        $resTotalLength = OomSserverSideProcess::sql_exec( $db,$count_request);
        ...
        return array(
            ...
            "data"            => OomSserverSideProcess::data_output( $columns, $data, $joinQuery )
        );
    }
    ...
    static function sql_exec ( $db, $bindings, $sql=null )
    {
        ...
        try {
            ...
            OomSserverSideProcess::fatal( "An SQL error occurred: ".$e->getMessage() );
        }
